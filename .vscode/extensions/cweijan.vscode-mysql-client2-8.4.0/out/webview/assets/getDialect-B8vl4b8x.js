...
FLUSH PRIVILEGES;
-- since mysql version 5.7, password column need change to authentication_string=PASSWORD("newPassword")
`}updateColumn(e,t){var n,r;let{nullable:i,extra:a}=t;return this.updateColumnSql({table:e,...t,isNotNull:i!=`YES`,isAutoIncrement:a==null||(n=a.toLowerCase())==null||(r=n.includes)==null?void 0:r.call(n,`auto_increment`)})}updateColumnSql(e){let{table:n,tableMeta:i,name:o,columnName:s=o,type:c,unsigned:l,zerofill:u,useCurrentTimestamp:d,isNotNull:f,isAutoIncrement:p,comment:m,defaultValue:h,generation_expression:g,character_set_name:_,collation_name:v}=e,y=c?.match(/char|text|set|enum|blob|binary/i)&&i?.collation!=v;return new a(`ALTER TABLE \`${n}\``).append(`\n\tCHANGE \`${s}\` \`${o}\` ${c}`).if(l==`1`,`UNSIGNED`).if(u==`1`,`ZEROFILL`).if(d,`ON UPDATE CURRENT_TIMESTAMP`).if(y&&_,`CHARACTER SET ${_}`).if(y&&v,`COLLATE ${v}`).if(f,`NOT NULL`).if(p,`AUTO_INCREMENT`).if(g,`GENERATED ALWAYS AS ${g} STORED`).condition(!g,e=>e.if(r(h)&&!f,`DEFAULT NULL`).if(!r(h),`DEFAULT ${h==`CURRENT_TIMESTAMP`?h:`${t(h,c)}`}`)).if(m,`COMMENT '${m}'`).append(`;`).toString()}showCollations(){return`select DEFAULT_COLLATE_NAME "name",CHARACTER_SET_NAME "charset",DESCRIPTION "description" from information_schema.CHARACTER_SETS ORDER BY name;`}showChecks(e,t){return`SELECT
